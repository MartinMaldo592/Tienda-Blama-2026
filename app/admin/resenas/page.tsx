...
export default function AdminResenasPage() {
  ...
  return (
    <m.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.4}} className="space-y-10 max-w-[1600px] mx-auto">
      <AdminPageHeader icon={<Star size={28} strokeWidth={1.5}/>} iconColor="bg-gradient-to-tr from-yellow-500 via-amber-500 to-yellow-600" iconShadow="shadow-yellow-500/20" title="Reseñas" totalItems={stats.total} totalLabel="reseñas totales" isFetching={isFetching} dotColor="bg-yellow-500"
        actions={<Button variant="outline" className="gap-2 haptic-scale shadow-sm rounded-xl h-11 px-5 font-bold border-slate-200 dark:border-slate-800" onClick={()=>{qc.invalidateQueries({queryKey:["adminResenas"]}); qc.invalidateQueries({queryKey:["adminResenasStats"]})}} disabled={isFetching}><RefreshCw className={`h-4 w-4 ${isFetching?'animate-spin':''}`}/>Sincronizar</Button>}
      />

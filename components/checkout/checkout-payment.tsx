    ...
    return (
        <div className="space-y-4 pt-4 border-t">
            ...
            <RadioGroup value={value} onValueChange={onChange} disabled={disabled} className="grid grid-cols-1 gap-3">
                ...
                <label
                    htmlFor="pm-wa"
                    className={`relative flex items-start space-x-3 border p-4 rounded-xl cursor-pointer transition-all active:scale-[0.98] ${value === 'whatsapp' ? 'border-green-600 bg-green-50/50 shadow-sm ring-1 ring-green-600' : 'hover:border-gray-300'} ${disabled ? 'opacity-50 pointer-events-none' : ''}`}
                >
                    <RadioGroupItem value="whatsapp" id="pm-wa" className="mt-1 text-green-600" />
                ...
                <label
                    htmlFor="pm-culqi"
                    className={`relative flex items-start space-x-3 border p-4 rounded-xl cursor-pointer transition-all active:scale-[0.98] ${value === 'culqi' ? 'border-blue-600 bg-blue-50/50 shadow-sm ring-1 ring-blue-600' : 'hover:border-gray-300'} ${disabled ? 'opacity-50 pointer-events-none' : ''}`}
                >
                    <RadioGroupItem value="culqi" id="pm-culqi" className="mt-1 text-blue-600" />
                    <div className="flex-1">
                        <div className="font-medium flex items-center gap-2 text-base">

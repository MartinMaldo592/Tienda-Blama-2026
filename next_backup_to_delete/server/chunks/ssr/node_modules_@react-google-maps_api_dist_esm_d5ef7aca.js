...
`),n+=`
*DETALLE DEL PEDIDO:*
`,h.forEach(a=>{n+=`• ${a.quantity} x ${a.nombre} - ${(0,b8.formatCurrency)(a.precio*a.quantity)}
`}),j>0&&l&&(n+=`
*SUBTOTAL: ${(0,b8.formatCurrency)(i)}*
*CUP\xd3N (${l}): -${(0,b8.formatCurrency)(j)}*`),n+=`
*TOTAL: ${(0,b8.formatCurrency)(k)}*`}function cc(a){let{orderIdFormatted:b,name:c,dni:d,phone:e,address:f,reference:g,locationLink:h,items:i,subtotal:j,discount:k,total:l,couponCode:m,shippingMethod:n}=a,o=`\xa1Hola! Soy ${c}. Quiero confirmar mi pedido: 🛍️
`;return o+=`📋 *Pedido #${b}*

*DATOS DE ENV\xcdO* 📦
`,n&&(o+=`🚚 *M\xe9todo:* ${"lima"===n?"Lima":"Provincia"}
`),o+=`👤 *Cliente:* ${c}
🪪 *DNI:* ${d}
📱 *Tel\xe9fono:* ${e}
📍 *Direcci\xf3n:* ${f}
`,g&&(o+=`🏠 *Referencia:* ${g}
`),h&&(o+=`🗺️ *Ubicaci\xf3n:* ${h}
`),o+=`
--------------------------------

*DETALLE DEL PEDIDO:*
`,i.forEach(a=>{let b=a.variante_nombre?` (${String(a.variante_nombre)})`:"";o+=`• ${a.quantity} x ${a.nombre}${b}
   Sub: ${(0,b8.formatCurrency)(a.precio*a.quantity)}
`}),o+=`
--------------------------------
`,k>0&&m&&(o+=`💵 *SUBTOTAL: ${(0,b8.formatCurrency)(j)}*
🏷️ *CUP\xd3N (${m}): -${(0,b8.formatCurrency)(k)}*
`),o+=`💰 *TOTAL A PAGAR: ${(0,b8.formatCurrency)(l)}*`}function cd(a,b){return`https://api.whatsapp.com/send/?phone=${encodeURIComponent(a)}&text=${encodeURIComponent(b)}&type=phone_number&app_absent=0`}function ce(a,b){return`/open-wa?phone=${encodeURIComponent(a)}&text=${encodeURIComponent(b)}&auto=1`}function cf(a){try{localStorage.setItem("blama_last_order_success",JSON.stringify({orderId:a,ts:Date.now()}))}catch(a){}try{let b=encodeURIComponent(JSON.stringify({orderId:a,ts:Date.now()}));document.cookie=`blama_last_order_success=${b}; Max-Age=1800; Path=/; SameSite=Lax`}catch(a){}}function cg(){try{localStorage.removeItem("cart-storage")}catch(a){}}a.s(["buildPreOpenUrl",()=>ce,"buildWhatsAppFinalMessage",()=>cc,"buildWhatsAppPreviewMessage",()=>cb,"buildWhatsAppUrl",()=>cd,"clearCartStorage",()=>cg,"normalizeDigits",()=>b9,"normalizeDni",()=>ca,"setLastOrderSuccessMarker",()=>cf],90078),a.s([],61368)}];
